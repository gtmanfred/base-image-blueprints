...
IMAGE_ID=$(curl -X GET -H 'Accept: text/plain' http://10.69.246.81/api/image_id/$CONFIG_LABEL)
/root/tmp/packages.sh > /root/tmp/packages.txt
curl -X POST -H 'Accept: application/json' -H "Content-Type: application/json" -d "$(cat /root/tmp/packages.txt)" http://10.69.246.81/api/pkg_info/$CONFIG_LABEL/$IMAGE_ID/pkg
...
wget http://10.69.246.205/nova-agent/nova-agent-FreeBSD-amd64-1.39.1.tar.gz
